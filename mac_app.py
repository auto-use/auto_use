...
def _setup_embedded_resources():
    ...
    try:
        from _embedded_resources import RESOURCES  # type: ignore - generated by pack_resources.py
    except ImportError:
        return False
...
def get_macos_use_path():
    """Get path to Auto_Use/macOS_use directory"""
    return get_auto_use_path() / "macOS_use"

def clean_scratchpad():
    """Clear all contents inside Auto_Use/macOS_use/scratchpad/ and sandbox_workspace/ on startup"""
    try:
        scratchpad_dir = get_macos_use_path() / "scratchpad"
...
def set_frontend_flag():
    """Override the FRONTEND flag in Auto_Use.macOS_use.tree.element to True"""
    try:
        from Auto_Use.macOS_use.tree import element
...
def request_macos_permissions():
    """Prompt user for required macOS permissions on first launch"""
...
def get_llm_providers():
    ...
    try:
        from Auto_Use.macOS_use.llm_provider.openrouter.view import MODEL_MAPPINGS as openrouter_models
        from Auto_Use.macOS_use.llm_provider.groq.view import MODEL_MAPPINGS as groq_models
        from Auto_Use.macOS_use.llm_provider.openai.view import MODEL_MAPPINGS as openai_models
        from Auto_Use.macOS_use.llm_provider.anthropic.view import MODEL_MAPPINGS as anthropic_models
        from Auto_Use.macOS_use.llm_provider.google.view import MODEL_MAPPINGS as google_models
        from Auto_Use.macOS_use.llm_provider.perplexity.view import MODEL_MAPPINGS as perplexity_models
...
def serve_static(filename):
    ...
    if IS_COMPILED:
        for prefix in ['frontend/', 'Auto_Use/macOS_use/sandbox/terminal/']:
...
def serve_terminal(filename):
    ...
    if IS_COMPILED:
        response = serve_embedded_file(f'Auto_Use/macOS_use/sandbox/terminal/{filename}')
        if response:
            return response
        return "Not found", 404

    terminal_path = os.path.join(os.path.dirname(__file__), 'Auto_Use', 'macOS_use', 'sandbox', 'terminal')
    ...
    try:
        ...
        def run_agent():
            ...
            def monitor_milestones():
                milestone_path = get_macos_use_path() / "scratchpad" / "milestone" / "milestone.md"
                last_pos = 0
...
def main():
    if "--cli-mode" in sys.argv:
        ...
        try:
            from Auto_Use.macOS_use.agent.cli.__main__ import main as cli_main
            cli_main()
        except Exception as e:
    ...
    # Prompt for required macOS permissions on first launch
    ...
    try:
        if sys.platform == 'darwin':
            ...
            center_x = int((screen_w - win_w) / 2)
            center_y = int((screen_h - win_h) / 2)
        else:
            import ctypes
    ...
    # Pre-init pynput keyboard on main thread (Carbon APIs require main dispatch queue)
    if sys.platform == 'darwin':
        try:
