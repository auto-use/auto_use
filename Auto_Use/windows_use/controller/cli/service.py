...
class CLIService:
    ...
    # Hard refusal threshold for view: don't even try to read files this large.
    ...
    # Cap each grep match line so a single minified file can't drown the result list
    ...
    def _resolve_search_base(self, path: str) -> Path:
        """Resolve the search base for grep/glob/view.
    ...
    def view(self, path: str, start: int = 0, end: int = 0) -> dict:
        ...
        try:
            ...
            # Read + tokenize. Direct read_text is faster than the previous
            # PowerShell Get-Content path and works identically on Windows.
